...
class HFRequest(Model):
    response: str  # This class holds the response from Hugging Face

# Create an agent for handling Hugging Face requests
hf_agent = Agent(name="hf_request_agent", seed="your_seed_phrase")

@hf_agent.on_message(model=HFRequest, replies=UAgentResponse)
async def handle_hf_request(ctx: Context, sender: str, msg: HFRequest):
    # Call your Hugging Face model here
    model = HuggingFaceEndpoint(repo_id="mistralai/Mixtral-8x7B-Instruct-v0.1", temperature=0.7, max_length=200)
    prompt = prompt_template.format(transactions=transactions_context, query=msg.response)
